...
$ceksoalesai = select($, 'soal', array('id_mapel' => $idm, 'jenis' => 2));
...
foreach ($ceksoalesai as $getsoalesai) {
    ...
    // $cekjwbesai = rowcount($, 'jawaban', $w2);
    // if ($cekjwbesai <> 0) {
    $getjwb2 = fetch($, 'jawaban', $w2);
    if ($getjwb2) {
...
$simpan = update($, 'nilai', $data, $where);
// if ($simpan) {
//     delete($, 'jawaban', $where);
// }
mysqli_query($, "INSERT INTO log (id_siswa,type,text,date) VALUES ('$ids','login','Selesai Ujian','$tanggal $waktu')");
